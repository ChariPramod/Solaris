...
async function run(parentId?: string) {
  ...
  throw new Error(`Job ${job.id} did not finish; inspect it before retrying.`);
}
...
assert.equal((await request(`/api/runs/${second}/attempts`)).parents.length, 1);
const comparison = await request(
  `/api/compare?candidate=${second}&baseline=${first}`,
);
assert.equal(comparison.summary.transitions.inconclusive, 0);
assert.equal(
  (await request(`/api/runs/${second}/gate`, { policy: { schema_version: 1 } }))
    .passed,
  false,
);
assert.equal(
